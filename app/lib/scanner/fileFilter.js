...
export function filterFilesForScanning(files, options = {}) {
    const { maxFiles = SCAN_CONFIG.MAX_FILES, maxFileSize = SCAN_CONFIG.MAX_FILE_SIZE_BYTES, } = options;
    const stats = {
    ...
    for (const file of files) {
        ...
        const isExcludedDir = Array.from(SCAN_CONFIG.EXCLUDED_DIRS).some((dir) => path.includes(`/${dir}/`) || path.startsWith(`${dir}/`));
        if (isExcludedDir) {
            stats.excluded++;
