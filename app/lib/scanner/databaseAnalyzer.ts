export interface DatabaseFinding {
  ...
  category: 'maintainability' | 'reliability' | 'performance' | 'security';
  file: string;
  lineStart: number;
  message: string;
  suggestedFix: string;
...
interface DbTableInfo {
  ...
  file: string;
  line: number;
  foreignKeys: Array<{ column: string; referencedTable: string; line: number }>;
...
function parseCreateTableName(line: string): string | null {
...
function parseForeignKey(line: string): { column: string; referencedTable: string } | null {
  const lower = line.toLowerCase();
  ...
  if (fkIndex < 0 || refIndex < 0 || refIndex < fkIndex) return null;
...
function parseIndexDefinition(line: string): { table: string; column: string } | null {
  const lower = line.toLowerCase();
  const onToken = ' on ';
  ...
  if (firstParen < 0 || closeParen < 0) return null;
...
function findLikelyQueryLines(content: string): Array<{ line: number; text: string }> {
...
export function analyzeDatabaseArtifacts(files: Array<{ path: string; content: string }>): DatabaseFinding[] {
  const sqlFiles = files.filter((f) => f.path.toLowerCase().endsWith('.sql'));
  const codeFiles = files.filter((f) =>
    ['.ts', '.tsx', '.js', '.jsx', '.sql'].some((ext) => f.path.toLowerCase().endsWith(ext))
  );
  ...
  for (const file of sqlFiles) {
    ...
    let currentTable: DbTableInfo | null = null;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (currentTable && lower.includes('foreign key') && lower.includes('references')) {
        ...
        if (fk) {
          currentTable.foreignKeys.push({ ...fk, line: i + 1 });
        }
      }
