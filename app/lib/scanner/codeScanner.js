...
import { filterFilesForScanning } from './fileFilter';
import { detectSmells } from './smellDetectors';
import { getScanLimits } from './scanConfig';
import { analyzeDatabaseArtifacts } from './databaseAnalyzer';
export async function scanRepositoryFiles(owner, repo, options = {}) {
    ...
    try {
        ...
        for (const file of scannable) {
            try {
                ...
                for (const smell of analysis.smells) {
                    const enriched = Object.assign(Object.assign({}, smell), { file: file.path });
                    result.smells.push(enriched);
                    if (smell.category === 'security')
                        result.securityIssues.push(enriched);
                    else if (smell.category === 'reliability')
                        result.bugs.push(enriched);
                    else
                        result.codeSmells.push(enriched);
                }
            }
            catch (_b) {
        ...
        for (const finding of dbFindings) {
            ...
            result.smells.push(enriched);
            if (enriched.category === 'security')
                result.securityIssues.push(enriched);
            else if (enriched.category === 'reliability')
                result.bugs.push(enriched);
            else
                result.codeSmells.push(enriched);
        }
