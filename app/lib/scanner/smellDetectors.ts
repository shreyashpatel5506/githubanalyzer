...
type Rule = (context: RuleContext) => Smell[];

function posToLine(sourceFile: ts.SourceFile, pos: number): number {
...
function analyzeMetrics(content: string, lines: string[], sourceFile: ts.SourceFile, language: string): FileMetrics {
  let codeLines = 0;
  let commentLines = 0;
  let blankLines = 0;
  let functions = 0;
  ...
  if (isCodeLanguage(language)) {
    const visit = (node: ts.Node, depth: number) => {
      if (
        ...
        const isAsync = modifiers?.some((m: ts.Modifier) => m.kind === ts.SyntaxKind.AsyncKeyword);
        if (isAsync) asyncFunctions++;
      }
...
const maintainabilityRules: Rule[] = [
  ({ metrics, sourceFile }) => {
    ...
    for (const [normalizedBody, lines] of functionBodies.entries()) {
      if (normalizedBody.length < 60) continue;
      if (lines.length > 1) {
        findings.push({
          id: 'DUPLICATE_LOGIC',
          severity: 'medium',
...
function runRules(rules: Rule[], context: RuleContext): Smell[] {
  const output: Smell[] = [];
...
export function detectSmells(content: string, filePath: string): AnalysisResult {
