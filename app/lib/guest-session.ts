 ...
 */
export function createGuestSession(email?: string): GuestSession {
 ...
 */
export function extractGuestToken(authHeader?: string): string | null {
 ...
 */
export function createClaimToken(guestId: string, email: string): string {
  const payload = {
    guestId,
    email,
    type: 'guest-claim',
 ...
 */
export function verifyClaimToken(token: string): { guestId: string; email: string } | null {
  try {
    const decoded = jwt.verify(token, JWT_SECRET, {
      algorithms: ['HS256'],
    }) as any;
