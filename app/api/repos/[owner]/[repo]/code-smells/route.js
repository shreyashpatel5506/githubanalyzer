...
export async function GET(req, { params }) {
    ...
    try {
        ...
        if (normalized.length === 0) {
            normalized = dedupeFindings(findingsFromSnapshot(latestSnapshot === null || latestSnapshot === void 0 ? void 0 : latestSnapshot.metrics, 'code_smells'));
        }
        ...
        for (const suggestion of publishedSuggestions || []) {
            ...
            const matches = summary.match(/\[\[RESOLVED:code-smell:([^\]]+)\]\]/g) || [];
            for (const marker of matches) {
                const idMatch = marker.match(/\[\[RESOLVED:code-smell:([^\]]+)\]\]/);
        ...
        return NextResponse.json({ smells: sortBySeverity(filtered), scannedBranch });
    }
    catch (error) {
