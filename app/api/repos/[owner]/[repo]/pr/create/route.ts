...
function buildResolutionMarker(type: FixType, issueId?: string) {
    if (!issueId || type === 'readme') return null;
    return `[[RESOLVED:${type}:${issueId}]]`;
}

function applyInlineFallbackPatch(baseContent: string, issue: IssuePayload | undefined, filePath: string) {
    if (!issue?.suggestedFix && !issue?.explanation) return baseContent;
