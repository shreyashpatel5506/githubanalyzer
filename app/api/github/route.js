...
function getAuthHeaders(token) {
    ...
    return Object.assign(Object.assign({}, publicHeaders), { Authorization: `Bearer ${token}` });
}
/* ================== HELPERS ================== */
...
async function fetchExactContributions(username, headers) {
  ...
  `;
    try {
        ...
        if (json.errors || !((_b = (_a = json === null || json === void 0 ? void 0 : json.data) === null || _a === void 0 ? void 0 : _a.user) === null || _b === void 0 ? void 0 : _b.contributionsCollection)) {
            return null;
        }
...
async function fetchAllRepos({ username, headers }) {
    ...
    while (repos.length < MAX_REPOS_TO_FETCH) {
        const res = await fetch(`${baseUrl}&per_page=${REPOS_PER_PAGE}&page=${page}&sort=updated`, { headers });
        ...
        if (result.data.length < REPOS_PER_PAGE)
            break;
    }
...
async function estimateCommits(username, repos) {
    ...
    for (const repo of repos.slice(0, 10)) {
        const res = await fetch(`${GITHUB_API}/repos/${username}/${repo.name}/commits?author=${username}&per_page=1`, { headers: publicHeaders });
        if (!res.ok)
            continue;
        const link = res.headers.get("link");
        if (link === null || link === void 0 ? void 0 : link.includes('rel="last"')) {
...
async function estimatePRBreakdown(username, repos) {
    ...
    let merged = 0;
    for (const repo of repos.slice(0, 10)) {
        const openRes = await fetch(`${GITHUB_API}/repos/${username}/${repo.name}/pulls?state=open&per_page=1`, { headers: publicHeaders });
        if (openRes.ok) {
        ...
        const closedRes = await fetch(`${GITHUB_API}/repos/${username}/${repo.name}/pulls?state=closed&per_page=30`, { headers: publicHeaders });
        if (closedRes.ok) {
...
export async function POST(req) {
    try {
        ...
        const profileResult = await handleGitHubResponse(profileRes, "Profile fetch failed");
        if (profileResult.error) {
            return NextResponse.json({ error: profileResult.message }, { status: profileResult.status });
        }
        ...
        if (authHeaders !== publicHeaders) {
            const [totalPRRes, mergedPRRes, openPRRes, graphStatsRaw] = await Promise.all([
                fetch(`${GITHUB_API}/search/issues?q=author:${cleanUsername}+type:pr&per_page=1`, { headers: authHeaders }),
                fetch(`${GITHUB_API}/search/issues?q=author:${cleanUsername}+type:pr+is:merged&per_page=1`, { headers: authHeaders }),
                fetch(`${GITHUB_API}/search/issues?q=author:${cleanUsername}+type:pr+is:open&per_page=1`, { headers: authHeaders }),
            ...
            graphStats = graphStatsRaw !== null && graphStatsRaw !== void 0 ? graphStatsRaw : {
