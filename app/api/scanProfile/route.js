...
import { GITHUB_API, GITHUB_GRAPHQL, getAuthHeaders, validateUsername, handleGitHubResponse, } from '../lib/githubhelper';
import { createAdminClient } from '@/app/lib/supabase';
import { getSessionUser } from '@/app/lib/auth-server';
import { PLAN_LIMITS } from '@/app/lib/billing';
import { resolveUserPlan } from '@/app/lib/entitlements';
import { createGuestSession, verifyGuestSession, extractGuestToken, createClaimToken } from '@/app/lib/guest-session';
...
export async function POST(req) {
    ...
    try {
        ...
        if (profileData.error) {
            return NextResponse.json({ error: profileData.message }, { status: profileData.status || 500 });
        }
        const githubUserId = (_a = profileData.data.id) === null || _a === void 0 ? void 0 : _a.toString();
        if (!githubUserId) {
            return NextResponse.json({ error: 'Invalid GitHub profile payload: missing user id' }, { status: 500 });
        }
        ...
        if (userId) {
            ...
            if (!existingScan) {
                ...
                if (profileScanCountError) {
                    return NextResponse.json({ error: `Failed to validate profile scan limit: ${profileScanCountError.message}` }, { status: 500 });
                }
        ...
        const { data: scannedProfile, error: profileError } = await supabase
            ...
            profile_scan_result: {
                ...
                stats: {
                    ...
                    activeRepositories: repos.filter((r) => new Date(r.pushed_at) > new Date(Date.now() - 30 * 24 * 60 * 60 * 1000)).length,
                },
        ...
        if (profileError) {
            ...
            return NextResponse.json({ error: 'Failed to store profile data', details: profileError.message }, { status: 500 });
        }
