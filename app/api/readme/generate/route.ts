...
function inferTechStack(filePaths: string[]): string[] {
    const stack = new Set<string>()
    const joined = filePaths.join('\n').toLowerCase()

    if (joined.includes('next.config')) stack.add('Next.js')
    if (joined.includes('package.json')) stack.add('Node.js')
    if (joined.includes('tsconfig')) stack.add('TypeScript')
    if (joined.includes('tailwind.config') || joined.includes('globals.css')) stack.add('Tailwind CSS')
    if (joined.includes('supabase')) stack.add('Supabase')
    if (joined.includes('clerk')) stack.add('Clerk')

    return [...stack]
}

function extractApiRoutes(filePaths: string[]): string[] {
    return filePaths
...
export async function POST(req: Request) {
    try {
        ...
        const filePaths = (treeResponse.data.tree || [])
            .filter((item: GitTreeItem) => item.type === 'blob' && Boolean(item.path))
            .map((item: GitTreeItem) => String(item.path))
