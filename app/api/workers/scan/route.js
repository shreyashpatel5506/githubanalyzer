...
function normalizeCodeSmellSeverity(value) {
...
async function ensureAndIncrementDetailedUsage(supabase, userId) {
...
function extractScore(text, label) {
    ...
    const match = text.match(regex);
    if (!match)
        return 7; // safe motivational default
...
function parseFixPlan(text) {
    ...
    return text
        ...
        .map((line) => {
        var _a;
        const time = ((_a = line.match(/Hour[s]?\\s*(.*?)(?:\\(|—|-))/i)) === null || _a === void 0 ? void 0 : _a[1]) || "Phase";
        return {
            phase: time,
            goal: line,
            deliverables: [],
...
function dedupeRows(rows, signature) {
    const seen = new Set();
...
export async function POST(req) {
    ...
    try {
        ...
        if (!repo)
            throw new Error("Repository data missing linked to scan");
        // 2. Prepare AI Prompt
        const prompt = `
...
`;
        ...
        if (scan.requested_by_user_id) {
            const { data: identity } = await supabase
                ...
                .limit(1)
                .maybeSingle();
            githubToken = (identity === null || identity === void 0 ? void 0 : identity.access_token) || null;
        }
...
Keep each array 3-8 items. Use realistic paths.`;
            ...
            (findings.code_smells || []).forEach((item) => {
            ...
            (findings.security_issues || []).forEach((item) => {
        ...
        if (finalCodeSmellRows.length > 0) {
            ...
            if (error)
                throw new Error(`Failed to insert code smells: ${error.message}`);
        }
        if (finalBugRows.length > 0) {
            const { error } = await supabase.from('bugs').insert(finalBugRows);
            if (error)
                throw new Error(`Failed to insert bugs: ${error.message}`);
        }
        if (finalSecurityRows.length > 0) {
            const { error } = await supabase.from('security_issues').insert(finalSecurityRows);
            if (error)
                throw new Error(`Failed to insert security issues: ${error.message}`);
        }
        const eslintTotalErrors = finalBugRows.length + finalSecurityRows.filter((r) => r.severity === 'high' || r.severity === 'critical').length;
        const eslintTotalWarnings = finalCodeSmellRows.length + finalSecurityRows.filter((r) => r.severity === 'medium' || r.severity === 'low').length;
